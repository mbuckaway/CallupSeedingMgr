...
def SetProxiesIfNecessary():
    ...
    if dProxies != {}:
        urllib.request.install_opener(urllib.request.build_opener(urllib.request.ProxyHandler(dProxies), poster.streaminghttp.StreamingHTTPSHandler()))
...
def VirusTotalSubmit(filenames, options):
	...
	with io.open('virustotal.html', 'w') as f:
		f.write( '<html>\n' )
		f.write( '<head>\n' )
